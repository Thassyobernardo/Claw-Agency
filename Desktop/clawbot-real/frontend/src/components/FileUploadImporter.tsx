...
export default function FileUploadImporter() {
  ...
  const handleUpload = async () => {
    setStatus("uploading");
    // Simulação de processamento de IA no Excel
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200 p-8 shadow-sm text-center">
      ...
      {!file ? (
        ...
        >
          <div className="h-16 w-16 bg-slate-50 rounded-full flex items-center justify-center text-slate-400 group-hover:text-aw-green transition">
            <Upload size={32} />
          </div>
          <div>
        ...
        <div className="border border-slate-200 rounded-2xl p-6">
          <div className="flex items-center gap-4 mb-6">
            <div className="h-12 w-12 bg-aw-green-light rounded-xl flex items-center justify-center text-aw-green">
              <FileText size={24} />
            </div>
          ...
          {status === "idle" && (
            <button 
              ...
              className="w-full bg-aw-green py-4 text-white font-bold rounded-xl shadow-lg shadow-aw-green/20 hover:bg-aw-green-dark transition"
            >
          ...
          {status === "uploading" && (
            <div className="flex flex-col items-center gap-3 py-2">
              ...
              <p className="text-sm font-bold text-aw-slate">AI is analyzing your transactions...</p>
            </div>
          )}

          {status === "success" && (
            <div className="flex flex-col items-center gap-3 py-2 text-aw-green">
              <CheckCircle size={32} />
              <p className="text-sm font-bold text-aw-slate">Success! {Math.floor(Math.random() * 50) + 10} transactions found.</p>
              <button 
