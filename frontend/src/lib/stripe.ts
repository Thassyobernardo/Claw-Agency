...
function getOrCreateStripe(): InstanceType<typeof Stripe> {
  if (globalThis.__ecolink_stripe) return globalThis.__ecolink_stripe;
  const key = process.env.STRIPE_SECRET_KEY;
  if (!key) throw new Error("STRIPE_SECRET_KEY is not set in .env.local");
  globalThis.__ecolink_stripe = new Stripe(key, { apiVersion: "2024-06-20" });
  return globalThis.__ecolink_stripe;
}

export const stripe: InstanceType<typeof Stripe> = new Proxy({} as InstanceType<typeof Stripe>, {
  get(_target, prop) {
