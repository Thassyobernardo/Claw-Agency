...
async function callClaude(prompt: string, jsonMode: boolean = true) {
  if (!OPENROUTER_KEY) {
...
export async function generateCarouselScript(newsContext: string, type: "global" | "local") {
  const prompt = `
...
export async function generateFreemiumScript(userText: string) {
...
export async function getReceptionistResponse(history: any[], newMessage: string) {
