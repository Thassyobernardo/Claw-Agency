...
function DashboardInner() {
  ...
  const [myobConnected, setMyobConnected] = useState<boolean | null>(null);
  const [benchmark,    setBenchmark]    = useState<BenchmarkData | null>(null);
  const [loading,      setLoading]      = useState(true);
  const [syncing,         setSyncing]         = useState(false);
  const [syncResult,      setSyncResult]      = useState<{ imported: number; skipped: number } | null>(null);
  const [myobSyncing,     setMyobSyncing]     = useState(false);
  const [myobSyncResult,  setMyobSyncResult]  = useState<{ imported: number } | null>(null);
  ...
  const loadData = useCallback(async () => {
    setLoading(true);
    try {
      const [s, t, c, x, b, m] = await Promise.all([
        ...
        fetch("/api/auth/myob/status").then(r => r.json()).catch(() => ({ connected: false })),
      ]);
      setSummary(s);
      ...
      setMyobConnected(m?.connected ?? false);
    } catch (e) {
      console.error(e);
    } finally {
      setLoading(false);
  ...
  const handleMyobSync = useCallback(async () => {
    setMyobSyncing(true);
    setMyobSyncResult(null);
    try {
      const res = await fetch("/api/myob/sync", { method: "POST" });
      const data = await res.json();
      if (res.ok) {
        setMyobSyncResult({ imported: data.imported });
        await loadData();
      } else {
        alert(data.message ?? "MYOB sync failed. Please try again.");
      }
    } catch {
      alert("Network error during MYOB sync.");
    } finally {
      setMyobSyncing(false);
    }
  }, [loadData]);
  ...
  return (
    <div className="min-h-screen bg-aw-gray/40">
      ...
      <header className="sticky top-0 z-40 border-b border-aw-gray-border bg-white/90 backdrop-blur-md">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
          ...
          <div className="flex items-center gap-3">
            ...
            {/* ── MYOB button ── */}
            {myobConnected === null ? null : myobConnected ? (
              <button
                onClick={handleMyobSync}
                disabled={myobSyncing}
                className="flex items-center gap-1.5 rounded-xl border border-blue-200 bg-blue-50 px-4 py-2.5 text-sm font-bold text-blue-700 transition-all hover:bg-blue-100 active:scale-95 disabled:opacity-60"
              >
                <RefreshCw size={14} className={myobSyncing ? "animate-spin" : ""} />
                {myobSyncing ? "Syncing…" : myobSyncResult ? `+${myobSyncResult.imported} MYOB` : "Sync MYOB"}
              </button>
            ) : (
              <Link href="/api/auth/myob" className="flex items-center gap-2 rounded-xl border border-aw-gray-border bg-white px-5 py-2.5 text-sm font-bold text-aw-slate transition-all hover:border-blue-400/60 hover:text-blue-600 active:scale-95">
                <Link2 size={16} /> Connect MYOB
              </Link>
            )}
      ...
      <main className="mx-auto max-w-7xl px-6 py-10 space-y-8">
        ...
        {/* ── Connect Xero CTA (hidden when connected) ──────────────────────── */}
        {!xeroStatus?.connected && (
          ...
          >
            ...
            <p className="text-aw-slate-mid font-medium mb-6 max-w-md mx-auto text-sm">
              Authorise EcoLink to read your Xero transactions and we'll classify every line item automatically.
            </p>
            <div className="flex items-center justify-center gap-4 flex-wrap">
              ...
              <button disabled className="flex items-center gap-2 rounded-xl border border-aw-gray-border bg-white px-8 py-3 text-aw-slate font-bold opacity-60">
                Connect MYOB
                <span className="rounded-md bg-aw-gray px-2 py-0.5 text-[11px] font-bold text-aw-slate-light">Soon</span>
              </button>
            </div>
          </motion.div>
        )}
